...
export class ClienteFormComponent {
  ...
  async saveCliente() {
    ...
    if (!this.cliente.nombre_completo.trim()) {
    ...
    if (this.cliente.dni && this.cliente.dni.trim() !== '') {
      ...
      const existe = await this.verificarDniExistente(this.cliente.dni, this.cliente.cliente_id);
      if (existe) {
      ...
      this.clienteService.getClientes().subscribe({
        next: (clientes) => {
          const duplicado = clientes.some(
            (c) => c.dni && c.dni === dni && c.cliente_id !== cliente_id
          );
          resolve(duplicado);
        },
        error: () => resolve(false)
